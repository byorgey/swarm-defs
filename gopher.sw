...
def drill_mound = \d. h <- scan d; if (h == inr "mound") {drill d; pure ()} {}
end

def gopher_turret =
  forever (x4 (turn left; drill_mound forward); drill_mound down)
end
...
def place_turret 
  = \row. \col.
  if (mod (2 * row + col) 5 == 0) {build {gopher_turret}; pure ()} {}
end
