...
def doboxP: ∀ a. Cmd a -> Dir -> (CU -> CU) -> (CU -> CU) -> Cmd Bool -> CU 
...
def harvestbox: Dir -> (CU -> CU) -> (CU -> CU) -> CU 
  = \d. \rep1. \rep2.
  doboxP harvest d rep1 rep2 notempty
end

def grabbox: Dir -> (CU -> CU) -> (CU -> CU) -> CU 
  = \d. \rep1. \rep2.
...
def plant_garden: Dir -> (CU -> CU) -> (CU -> CU) -> Text -> CU 
  = \d. \rows. \cols. \thing.
  waitFor thing;
  rows (
...
def garden: (CU -> CU) -> Dir -> (CU -> CU) -> (CU -> CU) -> Text -> CU 
  = \at. \d. \rows. \cols. \thing.
  planter <- build {at (plant_garden d rows cols thing)};
  give planter thing
end

def giveall: Actor -> Text -> CU = \r. \thing. while (has thing) {give r thing}
end
...
def make_with: CU -> Text -> CU = \get_ingrs. \thing. get_ingrs; make thing end
...
def tendbox: 
    (CU -> CU) -> Dir -> (CU -> CU) -> (CU -> CU) -> Text -> (CU -> CU) ->
    Actor -> CU 
...
def step3: Cmd Unit =
  ...
  make "big furnace";
  equip "big furnace";
  log "Please (1) harvest more trees, (2) grab >= 4 copper ore, then run step4"
end

def step4: Cmd Unit =
  log "Making copper wire and strange loops...";
  while (has "copper ore") {make "copper wire"};
  x16 (make "strange loop");
  log "Building lambda harvester...";
  build {tend "lambda" (\c. c)};
  log "Please (1) harvest more trees"
end